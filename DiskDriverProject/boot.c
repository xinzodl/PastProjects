...
int main ( int argc, char *argv[] )
{
    ...
    if (argc != 2)
    {
        printf("Usage: %s <number of_clients>\n", argv[0]) ;
        exit(-1) ;
    }
    ...
    fprintf(stderr, "BOOT: initializing the disk hardware...\n");
    diskhw_id = fork() ;
    switch (diskhw_id)
    {
        case -1: // error
    ...
    fprintf(stderr, "BOOT: initializing the pkernel...\n");
    ...
    fprintf(stderr, "BOOT: creating the clients\n");
	...
	fprintf(stderr, "BOOT: process %d returned %d, status %d\n", children[j], wait_return, status);
    }
